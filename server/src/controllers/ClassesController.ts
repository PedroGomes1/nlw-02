    ...
    const classes = await db('classes')
      .whereExists(function() {
        this.select('class_schedule.*')
          ...
          .whereRaw('`class_schedule`.`week_day` = ??', [Number(week_day)]) //Se o prof trabalha nesse dia
          .whereRaw('`class_schedule`.`from` <= ??', [timeInMinutos])
          .whereRaw('`class_schedule`.`to` >= ??', [timeInMinutos]) //Só pode marcar antes do horário que ele termina de trabalhar
        })
